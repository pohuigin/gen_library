...
;       Made sure r,g,b_curr get defined. TR, 07 May 2004
;-
...
if (!d.name eq 'X') then begin
    ...
    if (strmatch(visual,'GRAYSCALE',/FOLD_CASE) OR $
        strmatch(visual,'STATICGRAY',/FOLD_CASE)) then begin
...
; WHAT IF THE COLORS COMMON BLOCK HASN'T BEEN DEFINED YET...
...
if (N_elements(PLOTCOLORS) eq 0) then begin
   ...
   if decomposed or (not decomposed AND not keyword_set(NOCOLORS)) then begin
    ...
    if not keyword_set(START) then begin
        ...
        good = where(strmatch(plotcolors,'WHITE',/FOLD_CASE), ngood)
        ...
        good = where(strmatch(plotcolors,'BLACK',/FOLD_CASE), ngood)
        if (ngood ne 0) then begin
            plotcolors = plotcolors[good]
...
if not decomposed then begin 
    ...
    ; THE NEW COLOR TABLE...
    ; TR 3/19/04: DON'T USE IDL'S STRETCH, IT'S WRONG!
    if not keyword_set(START) then begin
        start = size-nplotcol
        ctindx = bytscl(findgen(size),MIN=0,MAX=start-1,TOP=size-1)
    endif else begin
        start = 0
        ctindx = bytscl(findgen(size),MIN=nplotcol,MAX=size-1,TOP=size-1)
    endelse

    ; SET THE CURRENT COLOR VECTORS IN THE COLOR COMMON BLOCK AND
    ; THEN LOAD THE CURRENT COLOR TABLE...
